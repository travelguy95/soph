...
@jit((Particle.class_type.instance_type,int64,int64,float64))
def border_collide(self,rows,cols,dt):
...
def run_code(rows,cols,steps,obstruct=np.array([[]]),obstruct_u=np.array([[]]),obstruct_v=np.array([[]])):
  ...
  for iter in range(0,steps+1):
    ...
    for count in range(0,10):
      for i, particle in enumerate(my_particles):
        border_collide(particle,rows,cols,dt)
        if obstruct[math.floor(particle.y)][math.floor(particle.x)] == 99:
          kinetic = 0.5*1*(particle.u**2+particle.v**2)**0.5
          impacts.append([time,int(math.floor(particle.y)),int(math.floor(particle.x)),kinetic])
    ...
    for times in range(0,10): # do this 10 times
      for i, particle in enumerate(my_particles):
        ...
        border_collide(particle,rows,cols,dt) #check again as particles moved due to collision
        if obstruct[math.floor(particle.y)][math.floor(particle.x)] == 99:
          kinetic = 0.5*1*(particle.u**2+particle.v**2)**0.5
          impacts.append([time,int(math.floor(particle.y)),int(math.floor(particle.x)),kinetic])
    list_of_list_positions = []
      ...
      for row in range(0,rows):
        for col in range(0,cols):
          if list_of_list_positions[row*cols+col][0] == -1 or obstruct[row][col] == 99:
            ...
            d = over_relax*(above-below+right-left)-(rho[row][col]-4) 
